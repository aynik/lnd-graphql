...
const subscribe = function (method, before, after) {
  return ({
    subscribe: (root, args) => {
      ...
      const callArgs = [{}, this.metadata]
      if (hasArgs) callArgs.reverse()
      const subscription = this[method].apply(this, callArgs)
      if (hasArgs) subscription.write(args)
    ...
    decodePaymentRequest: resolve.bind(connection,
      ...
      ({
        ...
        expiresOn: parseInt(expiry, 10) ?
          new Date(parseInt(timestamp + expiry, 10) * 1e3) : undefined,
        memo: description
      })
    ),
  ...
  Mutation: {
    ...
    sendPaymentSync: resolve.bind(connection,
      ...
      ({
        ...
        amt: amount,
        payment_hash_string: preimageHash,
