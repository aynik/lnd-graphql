...
const resolve = function (method, before, after, root, args) {
  args = before(args || {})
...
const subscribe = function (method, after) {
  const publish = (payload) => pubsub.publish(method, payload)
  const subscription = this[method]({}, this.metadata)
  subscription.on('data', publish).on('error', publish)
  return ({
    subscribe: () => pubsub.asyncIterator(method),
...
const buildResolvers = (connection) => ({
    ...
    listInvoices: resolve.bind(connection,
      ...
      ({ invoices }) => invoices.map((invoice) => ({
        ...
        createdOn: parseInt(invoice.creation_date, 10) ?
          new Date(parseInt(invoice.creation_date, 10) * 1e3) : null,
        settledOn: parseInt(invoice.settle_date, 10) ?
          new Date(parseInt(invoice.settle_date, 10) * 1e3) : null,
    ...
    lookupInvoice: resolve.bind(connection,
      ...
      ({
        ...
        createdOn: parseInt(creation_date, 10) ?
          new Date(parseInt(creation_date, 10) * 1e3) : null,
        settledOn: parseInt(settle_date, 10) ?
          new Date(parseInt(settle_date, 10) * 1e3) : null,
    ...
    listPayments: resolve.bind(connection,
      ...
      ({ payments }) => payments.map((payment) => ({
        ...
        createdOn: parseInt(payment.creation_date, 10) ? 
          new Date(parseInt(payment.creation_date, 10) * 1e3) : null,
        path: payment.path,
      ...
      ({ nodes, edges }) => ({
        nodes: nodes.map((node) => ({
          updatedOn: parseInt(node.last_update, 10) ? 
            new Date(parseInt(node.last_update, 10) * 1e3) : null, 
        ...
        edges: edges.map((edge) => ({
          ...
          updatedOn: parseInt(edge.last_update, 10) ? 
            new Date(parseInt(edge.last_update, 10) * 1e3) : null,
          publicKeys: {
            a: edge.node1_pub,
            b: edge.node2_pub
          },
    ...
    getChannelInfo: resolve.bind(connection,
      ...
      ({
        ...
        updatedOn: parseInt(last_update, 10) ? 
          new Date(parseInt(last_update, 10) * 1e3) : null,
        publicKeys: {
          a: node1_pub,
          b: node2_pub
        },
  ...
  Mutation: {
    ...
    openChannel: resolve.bind(connection,
      'OpenChannel',
      ({
        targetPeerId,
        nodePublicKey,
    ...
    closeChannel: resolve.bind(connection,
      'CloseChannel',
    ...
    sendPayment: resolve.bind(connection,
      'SendPayment',
      ({
        recipientPublicKey,
        amount,
    ...
    addInvoice: resolve.bind(connection,
      ...
      ({
        ...
        memo,
        receipt, 
        r_preimage: new Buffer(preimage, 'hex'),
        r_hash: new Buffer(preimageHash, 'hex'), 
        value: amount, 
        settled: isSettled,
        creation_date: ((createdOn.valueOf() / 1000) | 0).toString(),
        settle_date: settledOn ?
          ((settledOn.valueOf() / 1000) | 0).toString() : null,
    ...
    decodePayReq: resolve.bind(connection,
      'DecodePayReq',
      ({ payReq }) => ({ pay_req: payReq }),
      ({ destination, payment_hash, num_satoshis }) => ({
        destination,
